...
=======
/*   Updated: 2025/01/13 17:21:41 by victor-linu      ###   ########.fr       */
...
/* function ensures the map is fully enclosed by walls,
	contains exactly one player starting position,
...
=======
	...
	while (i < map_data->map_height)
	{
		...
		while (map_data->map_layout[i][j]
			&& ft_isspace(map_data->map_layout[i][j]))  // Skip leading spaces
			j++; 
		...
		if ((i == 0 || i == map_data->map_height - 1) &&
			(map_data->map_layout[i][j] != '1' || map_data->map_layout[i][len
					- 1] != '1')) // Checking for top row (row 0) and bottom row (last row)
			print_error("Error: Map is unenclosed by walls (top/bottom row)\n");
		if (i > 0 && i < map_data->map_height - 1 &&
			(map_data->map_layout[i][j] != '1' || map_data->map_layout[i][len
					- 1] != '1'))  // Checking left and right boundaries for middle rows
			print_error("Error: Map is unenclosed by walls (left/right column)\n");
		i++;
	}
	i = 0;
	player_count = 0;  // Check for a valid number of player positions
	while (i < map_data->map_height)
	{
		j = 0;
		while (map_data->map_layout[i][j])
