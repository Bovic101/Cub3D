...
/*   Updated: 2025/01/15 12:43:59 by taha             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++y < DISPLAY_HEIGHT)
	{
		...
		while (++x < DISPLAY_WIDTH)
		{
			if (y < DISPLAY_HEIGHT / 2)
				((uint32_t *)game->screen->pixels)[y * DISPLAY_WIDTH + x] = game->mlx_r->ceiling_c;
			else
				((uint32_t *)game->screen->pixels)[y * DISPLAY_WIDTH + x] = game->mlx_r->floor_c;
		}
	}
}
...
void	ft_cast_rays(t_game *game, int x, int y)
{
	while (++x < DISPLAY_WIDTH)
	{
		...
		game->rc->ray_dir_x = game->p->dir_x + game->p->plane_x * game->rc->camera_x;
		game->rc->ray_dir_y = game->p->dir_y + game->p->plane_y * game->rc->camera_x;
		game->rc->map_x = (int)(game->p->pos_x / BLOCK_SIZE);
		game->rc->map_y = (int)(game->p->pos_y / BLOCK_SIZE);
	if (game->rc->ray_dir_x == 0)
		game->rc->delta_dist_x = 1e30;
	else
		game->rc->delta_dist_x = fabs(1 / game->rc->ray_dir_x);
	if (game->rc->ray_dir_y == 0)
		game->rc->delta_dist_y = 1e30;
	else
		game->rc->delta_dist_y = fabs(1 / game->rc->ray_dir_y);
		if (game->rc->ray_dir_x < 0)
		{
			game->rc->step_x = -1;
			game->rc->side_dist_x = (game->p->pos_x / BLOCK_SIZE - game->rc->map_x) * game->rc->delta_dist_x;
		}
		else
		{
			game->rc->step_x = 1;
			game->rc->side_dist_x = (game->rc->map_x + 1.0 - game->p->pos_x / BLOCK_SIZE) * game->rc->delta_dist_x;
		}
		if (game->rc->ray_dir_y < 0)
		{
			game->rc->step_y = -1;
			game->rc->side_dist_y = (game->p->pos_y / BLOCK_SIZE - game->rc->map_y) * game->rc->delta_dist_y;
		}
		else
		{
			game->rc->step_y = 1;
			game->rc->side_dist_y = (game->rc->map_y + 1.0 - game->p->pos_y / BLOCK_SIZE) * game->rc->delta_dist_y;
		}
		ft_perform_dda(game);
...
void	ft_perform_dda(t_game *game)
{
		...
		while (game->rc->hit == 0)
		{
			...
			if (game->rc->map_x >= 0 && game->rc->map_y >= 0 && game->rc->map_y < game->mapdata->map_height && 
...
void	ft_render_wall(t_game *game, int x, int y)
{
		...
		{
			game->rc->color = 0xFF0000FF; // gonna change when textures add but now red walls.
			if (game->rc->side == 1) game->rc->color = 0x800000FF; // y side of walls
